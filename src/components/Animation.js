...
class Animation {
  constructor(config) {
    this.motionFn = config && config.motionFn ? config.motionFn : function () {
      return window.requestAnimationFrame.apply(window, arguments);
    };
    this.motionFnCancel = config && config.motionFnCancel ? config.motionFnCancel : function () {
    ...
    let callback = function () {
      ...
      if (config.hold >= config.duration) {
        ...
        config.el.forEach(function (node) {
          ...
          for (let p in config.to) {
            let reSty = '',
              sty, sty1;
            if (config.from == null || !(p in config.from)) {
              let _from = node.css(p);
            ...
            for (let i = 0; i < sty1.length; i++) {
              ...
              if (o.value.indexOf('#') == 0) { // rgb
                ...
                let t = config.hold;
                let b = sty == null ? 0 : parseFloat(sty[i].value);
                let c = sty == null ? parseFloat(sty1[i].value) : parseFloat(sty1[i].value) - parseFloat(sty[i].value);
            ...
            let reSty1 = reSty;
            let a = reSty.match(/rgba?\s*\((\d+\.?\d*\s*,?)+\s*\)/ig);
  ...
  identifyCssValue(cssStr) {
    ...
    if (typeof(cssStr) != 'undefined') {
      ...
      let a1 = cssStr.match(/([a-z\(,\s]*)([+\-]?\d+\.?\d*|#[\da-f]{6}|#[\da-f]{3})([a-z|%]*)([a-z\)]*)/ig);
      if (a1 != null) {
        let reSty = [];
        for (let i = 0; i < a1.length; i++) {
          let a = a1[i].match(/([a-z\(,\s]*)([+\-]?\d+\.?\d*|#[\da-f]{6}|#[\da-f]{3})([a-z|%]*)([a-z\)]*)/i);
          let sty = {
            style: a[0],
            prefix: a[1],
            value: a[2],
  ...
  setStyle() {
    if (arguments.length == 1) {
      ...
      for (let k = 0; k < config.el.length; k++) {
        let el = config.el[k];
  ...
  padZero(num, length) {
    ...
    let l1 = 0;
    do {
      l1 = re.indexOf('.') > -1 ? re.indexOf('.') : re.length;
      if (l1 < length) {
        re = '0' + re;
      }
    } while (l1 < length);
